...
export async function runSeeds() {
  ...
  await withTransaction(async (client) => {
    // 1. Seed Users with secure bcrypt hashing
    ...
    console.log(`✓ Seeded ${seedUsers.length} users with secure bcrypt hashes`);

    // 2. Seed Hotel Settings
    ...
    for (const r of seedRooms) {
      await client.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          description = EXCLUDED.description;
      `, [r.id, r.room_number, r.room_type_id, r.floor, r.price_per_night, r.status, r.housekeeping_status, r.description]);
    }
    ...
    console.log(`✓ Seeded ${seedStaff.length} staff members`);
    ...
    console.log(`✓ Seeded ${seedGuests.length} guests`);
    ...
    console.log(`✓ Seeded ${seedReservations.length} reservations`);
    ...
    console.log(`✓ Seeded ${seedHousekeepingTasks.length} housekeeping tasks`);

    // 9. Seed Invoices, Invoice Items & Payments
    ...
    console.log(`✓ Seeded ${seedInvoices.length} invoices, items, and payments`);
    ...
    console.log(`✓ Seeded ${seedAuditLogs.length} audit logs`);
  });
